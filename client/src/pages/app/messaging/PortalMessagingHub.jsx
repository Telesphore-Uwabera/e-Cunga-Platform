...
import { AlertIcon, BellIcon, CheckIcon, InboxIcon, MessageIcon, SentIcon, SparkIcon, SuccessIcon, SystemIcon, UnreadIcon, WarningIcon } from '../../../components/Icons.jsx';
...
function NotificationsFeed({ notifications, portalMessages, sentCount, markNotificationRead, showFlash, onSwitchToChat }) {
  ...
  const kindMeta = {
    alert:   { icon: <AlertIcon size={18} />,   label: 'Alert',   color: '#f59e0b', bg: 'rgb(254 243 199 / 0.8)' },
    message: { icon: <MessageIcon size={18} />, label: 'Message', color: '#3a6280', bg: 'rgb(224 242 254 / 0.8)' },
    system:  { icon: <SystemIcon size={18} />,  label: 'System',  color: '#692751', bg: 'rgb(243 232 255 / 0.8)' },
    warn:    { icon: <WarningIcon size={18} />,  label: 'Warning', color: '#dc2626', bg: 'rgb(254 226 226 / 0.8)' },
    ok:      { icon: <SuccessIcon size={18} />,  label: 'Success', color: '#16a34a', bg: 'rgb(220 252 231 / 0.8)' },
  };
  ...
  const filterOptions = [
    ...
    { id: 'alert',   label: <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><AlertIcon size={14} /> Alerts</span> },
    { id: 'message', label: <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><MessageIcon size={14} /> Messages</span> },
    { id: 'system',  label: <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><SystemIcon size={14} /> System</span> },
  ];
  ...
  return (
    <div className={styles.alertShell}>
      ...
      <div className={styles.notifStatsBar}>
        {[
          { icon: <InboxIcon size={20} />,   label: 'Total',  count: allNotifs.length,              bg: 'rgb(105 39 81 / 0.07)' },
          { icon: <UnreadIcon size={20} />,  label: 'Unread', count: unreadCount,                   bg: 'rgb(239 68 68 / 0.07)' },
          { icon: <SentIcon size={20} />,    label: 'Sent',   count: sentCount,                     bg: 'rgb(58 98 128 / 0.07)' },
          { icon: <CheckIcon size={20} />,  label: 'Read',   count: allNotifs.length - unreadCount, bg: 'rgb(22 163 74 / 0.07)' },
        ].map((s) => (
          <div key={s.label} className={styles.notifStatChip} style={{ background: s.bg }}>
            <span className={styles.notifStatIcon} style={{ color: 'var(--ec-primary)' }}>{s.icon}</span>
            <div>
      ...
      <div className={styles.notifFeed} aria-label="Notification feed">
        {visibleNotifs.length === 0 ? (
          <div className={styles.notifEmptyState}>
            <span className={styles.notifEmptyIcon} style={{ color: 'var(--ec-primary)' }}>
              <SuccessIcon size={48} />
            </span>
